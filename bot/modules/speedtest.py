...
async def speedtest(_, message):
    ...
    string_speed += f"<b>• My Ping:</b> <code>{result.ping} ms</code>\n"
    string_speed += f"<b>• My Upload:</b> <code>{get_readable_file_size(result.upload / 8)}/s</code>\n"
    string_speed += f"<b>• My Download:</b> <code>{get_readable_file_size(result.download / 8)}/s</code>\n"
    string_speed += f"<b>• My IP Address:</b> <code>{result.client['ip']}</code>"
